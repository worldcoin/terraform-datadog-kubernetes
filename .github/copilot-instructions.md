...
- When AI created or edited retained code, documentation, commits, pull request text including the title or body, or review replies, verify the applicable AI label when the repository provides it and that the body names each tool and publishes the actual initial user prompt plus material follow-up prompts. When exact text is genuinely unavailable, require an explicit gap disclosure and no reconstructed quote. A task summary, “AI assisted,” or chat link alone is insufficient.
- Respect explicit redactions of secrets, sensitive personal data, and confidential or private infrastructure and source evidence. Do not request hidden system/developer instructions, internal reasoning, unavailable private context, or reconstruction of unavailable prompt text.
